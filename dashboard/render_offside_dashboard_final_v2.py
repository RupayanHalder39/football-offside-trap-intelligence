...
def draw_radar(players_this_frame, ball_xy, attacking_team, defending_team, line_x, w, h):
    ...
    if line_x is not None and defending_team is not None:
        goal_x = 12000.0 if defending_team == 0 else 0.0
        x0, x1 = sorted([line_x, goal_x])
        p0, p1 = _pitch_xy_to_px(x0, 0), _pitch_xy_to_px(x1, 7000)
        # hatched "dangerous space behind the line" zone -- reads as a
        # distinct hazard region rather than a flat tint
        # vivid zone color, but light/sparse hatching (function defaults)
        # so it reads as a distinct hazard region without overpowering
        # the pitch-control wash, players, or the defensive line itself
        pitch_dark = draw_hatched_zone(pitch_dark, p0, p1, (0, 90, 255))
        lp0, lp1 = _pitch_xy_to_px(line_x, 0), _pitch_xy_to_px(line_x, 7000)
        cv2.line(pitch_dark, lp0, lp1, LINE_COLOR, 3, cv2.LINE_AA)
    for p in players_this_frame:
        ...
        if attacking_team is not None and p.get("vx_cm_s") is not None and p["display_team_id"] == attacking_team:
            ex, ey = int(px + p["vx_cm_s"] * 0.03), int(py + p["vy_cm_s"] * 0.03)
            cv2.arrowedLine(pitch_dark, (px, py), (ex, ey), (90, 235, 90), 2, cv2.LINE_AA, tipLength=0.3)
    ...
    panel_title(out, "OFFSIDE RADAR", sub="pitch control  •  defensive line  •  attacking runs", accent=LINE_COLOR)
    draw_radar_legend_box(out, w - 168, 40, 156, 172,
                           [("Team 0", TEAM_COLOR[0]), ("Team 1", TEAM_COLOR[1]),
                            ("Ball", (255, 255, 255)), ("Def. Line", LINE_COLOR)],
                           gradient=(TEAM_COLOR[0], TEAM_COLOR[1]),
                           gradient_labels=("Team 0", "Control", "Team 1"))
    return out


def draw_tiles_2x2(trap_info, lead_feat, defending_team, width, height):
    """2x2 KPI block (reference's left-column metrics layout, as
    opposed to pressing's single row of 4)."""
    cell_w = (width - 3 * GAP) // 2
    cell_h = (height - 3 * GAP) // 2
    img = np.full((height, width, 3), BG, dtype=np.uint8)
    trap_state = trap_info.get("trap_state", "UNCERTAIN") if trap_info else "UNCERTAIN"
    values = [
        ("Trap State", trap_state.replace("_", " ").title(), f"Team {defending_team}" if defending_team is not None else "n/a", TRAP_STATE_COLOR.get(trap_state, ACCENT_ORANGE), "shield"),
        ("Trap Synchrony", f"{trap_info['score']:.2f}" if (trap_info and trap_info.get("valid")) else "N/A", "0-1 scale", ACCENT_ORANGE, "target"),
        ("Distance to Line", f"{lead_feat['relation_cm']/100:+.1f} m" if lead_feat else "N/A", lead_feat["relation_label"] if lead_feat else "n/a", ACCENT_CYAN, "ruler"),
        ("Relative Speed to Line", f"{lead_feat['closing_rate_cm_s']/100:+.1f} m/s" if (lead_feat and lead_feat["closing_rate_cm_s"] is not None) else "N/A", "closing rate", ACCENT_MAGENTA, "bolt"),
    ]
...
def draw_trap_state_distribution(width, height, dir0, dir1):
    """Real, whole-clip trap-state frame-fraction distribution
    (combined both directions) -- the 'event/trap-state distribution'
    panel from the reference, built entirely from already-computed V2
    trap_state_series output."""
    from collections import Counter
    counts = Counter()
    for d in (dir0, dir1):
        counts.update(r["trap_state"] for r in d["trap_by_frame"].values())
    total = sum(counts.values()) or 1
    img = panel_frame(width, height)
    panel_title(img, "Trap State Distribution", sub="both directions combined", accent=ACCENT_ORANGE)
...
def main():
    ...
    def render_frame(f):
        ...
        d = dir_by_defender[defending_team] if defending_team is not None else None

        line_info = compute_offside_line(cleaned, defending_team, f) if defending_team is not None else {"valid": False, "reason": "possession uncertain"}
        line_x = line_info["line_x"] if line_info.get("valid") else None
        trap_info = d["trap_by_frame"].get(f) if d else None
        if trap_info is None:
            trap_info = {"valid": False, "reason": "possession uncertain", "score": None, "trap_state": "UNCERTAIN"}
        ...
        if defending_team is not None:
            banner_text = trap_state.replace("_", " ").title()
            banner_color = TRAP_STATE_COLOR.get(trap_state, (100, 100, 100))
            role_text = f"Team {defending_team} defends vs Team {attacking_team} attacks"
        else:
            banner_text, banner_color, role_text = "UNCERTAIN", (90, 90, 90), "Defending/Attacking: Uncertain"

        left = cv2.resize(draw_left_feed(frame, tracking_by_frame.get(f, []), attacking_team, defending_team,
                                          line_x, transformer, lead["track_id"] if lead else None, run_track_ids,
        ...
        kpi_block = draw_tiles_2x2(trap_info, lead_feat, defending_team, KPI_COL_W, CONTENT_H)
        ...
        cl_bot = draw_big_graph(GRAPH_COL_W, PANEL_H, hist_lead_depth, cur_time, GRAPH_WINDOW_SEC,
                                  "Distance to Lead Attacker", y_depth, "m", LEAD_COLOR)
        col_gap = np.full((GAP, GRAPH_COL_W, 3), BG, dtype=np.uint8)
        center_left_col = np.vstack([cl_top, col_gap, cl_bot])

        dist_panel = draw_trap_state_distribution(GRAPH_COL_W, PANEL_H, dir_by_defender[0], dir_by_defender[1])
        pattern_lines = [f"{len(all_runs)} attacking runs detected (both directions).",
                          f"Run types: {agg[0]['n_runs_against']} vs Team 0's line, {agg[1]['n_runs_against']} vs Team 1's line.",
                          f"{len(all_cands)} trap-break candidate(s) (needs TRAP_ACTIVE + a run threatening the line).",
                          (f"Trap synchrony peaks {max(agg[0]['max_trap_score'] or 0, agg[1]['max_trap_score'] or 0):.2f} "
                           f"but never sustains to TRAP_ACTIVE on this clip." if not all_cands else "See timeline markers for candidate timing.")]
        pattern_panel = draw_insights_panel(GRAPH_COL_W, PANEL_H, "Trap / Run Pattern Summary", pattern_lines, accent=ACCENT_ORANGE)
